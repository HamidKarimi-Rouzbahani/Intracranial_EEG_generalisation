...
ictal_or_inter='interictal'; % 'ictal' or 'interictal' data analysis
% Data recordings for each patint
for Patient=[1:56]
    if Patient==1
...
%         inter_ictal_trials=[1:2];        
    ...
    % load each trial/epoch/recording
    for trial=trials
        ...
        % excluding bad, ECG/EKG channels and channels with n/a labels
        ch=0;
        ...
        % Keeping the whole recording
        signal=data.trial{1,1};

        % Saving the signals and channel information
        if strcmp(ictal_or_inter,'ictal')
