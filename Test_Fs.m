...
for Patient=1:56    
    ...
    % pre_siz_analysis=60000;  %ms
    % post_siz_analysis=126999; %ms
    if strcmp(ictal_or_inter,'ictal')
        pre_siz_analysis=15000;  %*2ms
        post_siz_analysis=15000; %*2ms
        window_spans=[10 100 500]; %*2ms
        step_size=100; % *2ms
    elseif strcmp(ictal_or_inter,'interictal')
        pre_siz_analysis=1;  %*2ms
        %     post_siz_analysis=30000; %*2ms
        post_siz_analysis=150000; %*2ms
    ...
    Time_windows=[round((max(window_spans)/step_size)/2):floor((length(windows)/step_size-(max(window_spans)/step_size)/2))]; % e.g., 2:300 sampled time points
...
%                 clc
%                 [Patient trial]
                display('signal shorter than 5 mins!')
