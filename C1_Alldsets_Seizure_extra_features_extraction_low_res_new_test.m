...
for Patient=29:43
    ...
    for trial=trials
        ...
        if strcmp(ictal_or_inter,'ictal')
            ...
            step_size=round(1500*(Fs/500)); % /FS ms; 1000ms
            ...
            window_spans=round([1500]*(Fs/500)); %/FS ms; 20,200 and 1000 ms
        ...
        elseif strcmp(ictal_or_inter,'interictal')
            ...
            window_spans=round([1500]*(Fs/500)); %/FS ms; 200,1000 and 2000 ms
            try
            ...
            Time_windows=[ceil((max(window_spans)/step_size)/2):floor((length(windows)/step_size-(max(window_spans)/step_size)/2))]; % e.g., 2:300 sampled time points
            %             Time_windows=randsample([min(Time_windows):max(Time_windows)],10);
            Time_windows=round(linspace(min(Time_windows),max(Time_windows),10));
        end
        channels=[1:length(channels_id)];
        features = nan(max(featuress),length(channels),length(Time_windows), length(window_spans));
        %         load([Patient_initials,'_All_Features_Seizure',num2str(trial),chars,'.mat'],'features');
        t=0;
                ...
                for feature=featuress % features when evaluating time windows
                    ...
                    for channel = channels
                        ...
                        elseif feature==6
                            ...
                            %         threshold = mean(data);
                            trial_data_tmp=trial_data;
                            trial_data_tmp(trial_data>= threshold)=1;
                            trial_data_tmp(trial_data< threshold)=0;
                            [features(feature,ch,t,ws),~] = calc_lz_complexity(trial_data_tmp, 'exhaustive', 1);
                        ...
                        elseif feature==30
                            ...
                            %                             ICC_tmp=zeros(size(signal,1),1);
                            %                             for ch2=1:size(signal,1)
                            %                                 trial_data2=signal(ch2,wind);
                            %                                 ICC_tmp(ch2,1)=corr(trial_data',trial_data2');
                            %                             end
                            %                             left_chans=1:size(signal,1);
                            %                             left_chans(ch)=[];
                            if cor_done==0
                        ...
                        elseif feature>30
                            if coh_done==0
                                if Fs==256
                                    ICcoh_tmp=nan(size(signal,1),size(signal,1),49);
                                else
                            ...
                            elseif feature==33
                                % Alpha band
                                if Fs==256
                                    features(feature,ch,t,ws)=squeeze(nanmean(ICcoh([4:6])));
                                else
                                    features(feature,ch,t,ws)=squeeze(nanmean(ICcoh([3:4])));
                                end
                            elseif feature==34
                                % Beta band
                                if Fs==256
                                    features(feature,ch,t,ws)=squeeze(nanmean(ICcoh([6:12])));
                                else
                            ...
                            elseif feature==35
                                % Gamma band
                                if Fs==256
                                    features(feature,ch,t,ws)=squeeze(nanmean(ICcoh([13:35])));
                                else
                            ...
                            elseif feature==36
                                % High gamma band
                                if Fs==256
                                    features(feature,ch,t,ws)=squeeze(nanmean(ICcoh([36:49])));
                                else
        ...
        if strcmp(ictal_or_inter,'ictal')
            save([Patient_initials,'_Full_Features_Seizure',num2str(trial),chars,'.mat'],'features','channels_id',...
                'pre_siz_analysis','post_siz_analysis','window_spans','step_size','featuress',...
                'channels','windows','Time_windows','channel_info')
        elseif strcmp(ictal_or_inter,'interictal')
            save([Patient_initials,'_Full_Features_Interictal',num2str(trial),chars,'.mat'],'features','channels_id',...
                'pre_siz_analysis','post_siz_analysis','window_spans','step_size','featuress',...
                'channels','windows','Time_windows','channel_info')
        end
        toc
